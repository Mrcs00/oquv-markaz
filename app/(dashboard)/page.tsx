...
import { READY_POOL_SIZE, SHIFT_META } from "@/lib/constants";
  ...
  return (
    <div>
      ...
      <div className="p-4 md:p-8 space-y-6 md:space-y-8">
        ...
        {/* Ochilgan, lekin hali to'lmagan haqiqiy guruhlar */}
        {stats.gatheringGroups.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-slate-900 mb-3">Guruhlar yig'ilmoqda</h2>
            <div className="grid md:grid-cols-2 gap-4">
              {stats.gatheringGroups.map((g) => {
                const shiftMeta = SHIFT_META[g.shift];
                const pct = Math.min(100, (g.currentCount / g.max_students) * 100);
                return (
                  <Link
                    key={g.id}
                    href={`/groups/${g.id}`}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-primary-50 text-primary-700 shrink-0">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary-500" />
                        Yig'ilmoqda
                      </span>
                    </div>
                    ...
                    <div className="mt-4">
                      ...
                      <p className="text-xs text-slate-400 mt-1.5">
                        Batafsil va "Guruhni ochish" — guruh sahifasida.
                      </p>
                    </div>
                  </Link>
                );
